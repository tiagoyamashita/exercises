/**
 * Top-right logged-in user panel (and optional header login form) for exercises dashboards.
 * Requires #header-user-panel; #header-login-form is optional when using a full-page auth gate.
 */
(function (global) {
  ...
  function updateHeaderAuthPanel(data) {
    ...
    if (isLoggedInSession(data)) {
      setLoginError("");
      if (form) setVisible(form, false, "flex");
    ...
    if (form) setVisible(form, true, "flex");
  }
